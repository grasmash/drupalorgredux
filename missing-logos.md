# Missing Ticker Logos — SVG Markup

All logos below are ready to paste into the ticker. Each renders at height="18" using `fill="currentColor"` to inherit the ticker's color scheme.
...
| Company | Source | Type |
|---------|--------|------|
| UNICEF | Text wordmark | SVG `<text>` — bold sans-serif, all caps, wide tracking |
| Pfizer | Text wordmark | SVG `<text>` — light sans-serif |
| Harvard | Text wordmark | SVG `<text>` — Georgia serif |
| The Economist | Text wordmark | SVG `<text>` — Georgia serif |
| Johnson & Johnson | Text wordmark | SVG `<text>` — Georgia serif italic |
| Charles Schwab | Text wordmark | SVG `<text>` — bold sans-serif |
| Amnesty International | Text wordmark | SVG `<text>` — bold sans-serif, all caps |
| The Weather Channel | Simple Icons | Vector path `<path>` — official monochrome icon |
| University of Oxford | Text wordmark | SVG `<text>` — Georgia serif |

## Notes

- **The Weather Channel** is the only one available as an official vector icon from Simple Icons. It renders as a 24x24 monochrome path-based SVG that matches the existing NASA/Tesla/BBC/Pinterest/GE icons perfectly.
- The other 8 use SVG `<text>` wordmarks. These render using system fonts and will inherit `currentColor` from the ticker CSS. They are styled to look intentional as wordmarks rather than plain text.
- Full-color vector logos were found on Worldvectorlogo for UNICEF, Pfizer, Johnson & Johnson, Amnesty International, Charles Schwab, and Harvard, but these are complex multi-color SVGs that would not work in a monochrome `fill="currentColor"` ticker context.
- The `viewBox` dimensions are approximate and may need fine-tuning depending on the actual font metrics in the browser. The key dimension is `height="18"` which keeps them consistent with the existing icon logos.
- For the text-based SVGs, `fill="currentColor"` is set on both the `<svg>` and the `<text>` element to ensure proper color inheritance from the `.proof-ticker-logo` CSS.
